...
qsv join --left position cpssa.csv raw reconciled-positions.csv | qsv select minister,position\[1\],raw,startdate,enddate | qsv join --left minister - name reconciled-people.csv | qsv select id,minister,position,raw,startdate,enddate | qsv rename item,name,positionItem,position,start,end | sed -e 's/2019-12-31//' > scraped.csv
